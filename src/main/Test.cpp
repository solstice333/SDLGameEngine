...
int main(int argc, char* argv[]) {
	if (TEST_GRAPHICS) {
		...
		CircFigure cf2(900, 350, dot, screen, Figure::GRAVITY_ENABLED, false, 0, 0, 0, 1, LEVEL_WIDTH, LEVEL_HEIGHT,
				&red, &green, &blue, &shimmer);
		RectFigure coin1(600, 325, coin, screen, Figure::GRAVITY_DISABLED, false, 0, 0, 0, 1, LEVEL_WIDTH,
				LEVEL_HEIGHT);
		...
		if (FOO) {
			rf.setFigure(100, 300, foo, screen, gravEnDis, true, FS, G, FJS, FNC, LEVEL_WIDTH, LEVEL_HEIGHT, &red,
					&green, &blue, &shimmer);
			rf.resolution = rf.player;
		}
		else cf.setFigure(100, 300, dot, screen, gravEnDis, true, CS, G, CJS, CNC, LEVEL_WIDTH, LEVEL_HEIGHT);
		...
		while (!quit) {
			...
			if (FOO) {
				...
				for (int i = 0; i < (int) collisions.size(); i++) {
					...
					if (marker == 2) removeIndex(collisions, i);
				}
			}
			else {
